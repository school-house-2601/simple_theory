...
const SKILL_TIPS = {
  Theory:
    "Try watching a music theory video to understand scales and chords better.",
...
export default function SkillRadar() {
  ...
  const [data, setData] = useState(
    ALL_SKILLS.map((s) => ({ skill: s, value: 0 })),
  );
  ...
  useEffect(() => {
    ...
    const fetchSkills = () => {
      fetch(`${import.meta.env.VITE_API_URL}/stats/skills/${user.id}`)
        .then((res) => res.json())
        .then((rows) => {
          ...
          const merged = ALL_SKILLS.map((skill) => {
            ...
            const value = match
              ? Math.min(
                  Math.round((Number(match.total_xp) / MAX_XP_PER_SKILL) * 100),
                  100,
                )
              : 0;
            return { skill, value };
        ...
        <div className="skill-panels">
          <div className="skill-panel">
            ...
            <div>
              ...
              <p>
                {focusSkill
                  ? SKILL_TIPS[focusSkill] ||
                    `Work on your ${focusSkill} skills today.`
