...
export default function FallingNotes({ notes = [], currentIndex = 0, detectedNote, instrument }) {
    ...
    const renderGuitarStyle = () => (
        <div className="guitar-strings-track">
            {notes.map((note, i) => {
                ...
                return (
                    <div key={i} className={`guitar-string-row ${isActive ? "active" : ""}`}>
                        <div className={`string-circle ${isCompleted ? "completed" : ""} ${isDetected ? "detected" : ""}`}
                            style={{ borderColor: theme.color, backgroundColor: isCompleted ? "#4caf7d" : isActive ? theme.color : "transparent"}}
                        >
                            {getLabel(note)}
                        </div>
                        <div className="string-line" style={{ backgroundColor: isCompleted ? "#4caf7d" : isActive ? theme.color : "#3b3b6b" }} />
                        {isCompleted && <span style={{ color: "#4caf7d", fontSize: "0.8rem" }}>✓</span>}
                    </div>
                );
            })}
        </div>
                    ...
                    <div key={i} className="vocal-bubble-wrapper">
                        <div 
                            ...
                            style={{
                                ...
                                backgroundColor: isCompleted ? "#4caf7d" : isActive ? `${theme.color}33` : "transparent",
                                boxShadow: isActive ? `0 0 20px ${theme.color}66` : "none",
                            }}
                        >
                            <span className="bubble-note">{getLabel(note)}</span>
                        </div>
                        {isCompleted && <span style={{ color: "#4caf7d", fontSize: "0.75rem" }}>✓</span>}
                    </div>
                );
            })}
        </div>
    ...
    const renderDrumsStyle = () => (
        <div className="drums-track">
            {notes.map((beat, i) => {
                const isActivate = i === currentIndex;
                const isCompleted = i < currentIndex;

                return (
                    <div
                        key={i}
                        className={`drump-pad ${isActive ? "active" : ""} ${isCompleted ? "completed" : ""}`}
                        style={{
                            borderColor: isActive ? theme.color : "#3b3b6b",
                            backgroundColor: isCompleted ? "#4caf7d33"
                                : isActive ? `${theme.color}33` : "#1e1e3a",
                            boxShadow: isActive ? `0 0 20px ${theme.color}66` : "none",
                        }}
                    >
                        <span className="drum-icon">{isCompleted ? "✓" : isActive ? "🥁" : beat}</span>
                    </div>
                );
            })}
        </div>
    ...
    const renderTrack = () => {
        switch (instrument) {
            case "Piano": return renderPianoStyle();
            case "Guitar": return renderGuitarStyle();
            case "Vocals": return renderVocalsStyle();
            case "Drums": return renderDrumsStyle();
            default: return renderPianoStyle();
        }
    };
    ...
    return (
        <div className={`falling-notes ${theme.style}`}>
            ...
            {renderTrack()}
            <div className="current-note-display" style={{ borderColor: `${theme.color}44` }}>
                <span className="current-label">
                    {instrument === "Vocals" ? "Sing:" : instrument === "Drums" ? "Beat:" : "Play:"}
                </span>
                ...
                {detectedNote && (
                    <span className="detected-note">
                        Detected: <strong style={{ color: theme.color }}>{detectedNote}</strong>
                    </span>
                )}
            </div>
        </div>
